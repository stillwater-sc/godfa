package root

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the generate command

import (
	"net/http"

	middleware "github.com/go-openapi/runtime/middleware"

	"github.com/stillwater-sc/godfa/api/models"
)

// VersionHandlerFunc turns a function with the right signature into a version handler
type VersionHandlerFunc func(VersionParams, *models.Principal) middleware.Responder

// Handle executing the request and returning a response
func (fn VersionHandlerFunc) Handle(params VersionParams, principal *models.Principal) middleware.Responder {
	return fn(params, principal)
}

// VersionHandler interface for that can handle valid version params
type VersionHandler interface {
	Handle(VersionParams, *models.Principal) middleware.Responder
}

// NewVersion creates a new http.Handler for the version operation
func NewVersion(ctx *middleware.Context, handler VersionHandler) *Version {
	return &Version{Context: ctx, Handler: handler}
}

/*Version swagger:route GET / root version

API version

Return the version of the DFA API


*/
type Version struct {
	Context *middleware.Context
	Handler VersionHandler
}

func (o *Version) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	route, _ := o.Context.RouteInfo(r)
	var Params = NewVersionParams()

	uprinc, err := o.Context.Authorize(r, route)
	if err != nil {
		o.Context.Respond(rw, r, route.Produces, route, err)
		return
	}
	var principal *models.Principal
	if uprinc != nil {
		principal = uprinc.(*models.Principal) // this is really a models.Principal, I promise
	}

	if err := o.Context.BindValidRequest(r, route, &Params); err != nil { // bind params
		o.Context.Respond(rw, r, route.Produces, route, err)
		return
	}

	res := o.Handler.Handle(Params, principal) // actually handle the request

	o.Context.Respond(rw, r, route.Produces, route, res)

}
